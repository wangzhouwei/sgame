...
Alert.show = function (detailString, enterCallBack, needCancel, animSpeed) {
    ...
    cc.loader.loadRes("prefab/Alert", cc.Prefab, function (error, prefab) {
        ...
        self.actionFadeIn = cc.sequence(cc.spawn(cc.fadeTo(Alert._animSpeed, 255),cc.scaleTo(Alert._animSpeed, 1.0).easing(cc.easeBackOut())), cbFadeIn);
        self.actionFadeOut = cc.sequence(cc.spawn(cc.fadeTo(Alert._animSpeed, 0), cc.scaleTo(Alert._animSpeed, 0).easing(cc.easeBackIn())), cbFadeOut);
        // 获取子节点
        Alert._detailLabel = cc.find("background/detailLabel", alert).getComponent(cc.Label);
        Alert._cancelButton = cc.find("background/centerbtn", alert);
        Alert._enterButton = cc.find("background/surebtn", alert);
        Alert._closeButton = cc.find("background/closebtn", alert);
